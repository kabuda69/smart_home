...
export default {
  data: function() {
    return {
      showAddDialog: false, showThresholdDialog: false, showHistoryDialog: false,
      snackbar: { show: false, text: '', color: 'success' },
      deviceTypes: [], newDevice: { name: '', typeId: null, thresholdMin: null, thresholdMax: null },
      thresholdForm: { deviceId: null, deviceName: '', min: 0, max: 100 },
      sliderValues: {}, historyData: []
    }
  },
  computed: { ...mapState(['devices']) },
  watch: { devices: { handler: function(d) { var self = this; d.forEach(function(x) { self.$set(self.sliderValues, x.id, x.currentValue || 0) }) }, immediate: true, deep: true } },
  mounted: function() { var self = this; this.$store.dispatch('fetchDevices'); this.$axios.get('/api/device-types').then(function(r) { if (r.data.success) self.deviceTypes = r.data.data }) },
  methods: {
    getIcon: function(t) { var m = { '智能灯': 'mdi-lightbulb', '空调': 'mdi-air-conditioner', '温度传感器': 'mdi-thermometer', '湿度传感器': 'mdi-water-percent', '智能插座': 'mdi-power-socket', '门窗传感器': 'mdi-door', '烟雾报警器': 'mdi-smoke-detector', '智能窗帘': 'mdi-blinds' }; return m[t] || 'mdi-devices' },
    hasValueControl: function(t) { return ['智能灯', '空调', '智能窗帘'].indexOf(t) !== -1 },
    isSensor: function(t) { return ['温度传感器', '湿度传感器', '烟雾报警器'].indexOf(t) !== -1 },
    getSensorUnit: function(t) { return { '温度传感器': '°C', '湿度传感器': '%', '烟雾报警器': 'ppm' }[t] || '' },
    simulateSensor: function(d) { var self = this, v = Math.floor(Math.random() * 30) + 15; this.$axios.post('/api/devices/command', { deviceId: d.id, commandType: 'SET_VALUE', commandValue: String(v) }).then(function() { self.$store.dispatch('fetchDevices'); self.showMsg('模拟数据: ' + v, 'success') }) },
    togglePower: function(d) { var self = this, cmd = d.powerState ? 'POWER_OFF' : 'POWER_ON'; this.$axios.post('/api/devices/command', { deviceId: d.id, commandType: cmd }).then(function() { self.$store.dispatch('fetchDevices'); self.showMsg(cmd === 'POWER_ON' ? '已开启' : '已关闭', 'success') }) },
    setValue: function(d) { var self = this, v = this.sliderValues[d.id]; this.$axios.post('/api/devices/command', { deviceId: d.id, commandType: 'SET_VALUE', commandValue: String(v) }).then(function() { self.$store.dispatch('fetchDevices'); self.showMsg('已设置: ' + v, 'success') }) },
    addDevice: function() { var self = this; this.$axios.post('/api/devices', this.newDevice).then(function(r) { if (r.data.success) { self.showAddDialog = false; self.newDevice = { name: '', typeId: null, thresholdMin: null, thresholdMax: null }; self.$store.dispatch('fetchDevices'); self.showMsg('添加成功', 'success') } }) },
    deleteDevice: function(id) { var self = this; if (!confirm('确定删除？')) return; this.$axios.delete('/api/devices/' + id).then(function() { self.$store.dispatch('fetchDevices'); self.showMsg('已删除', 'success') }) },
    openThresholdDialog: function(d) { this.thresholdForm = { deviceId: d.id, deviceName: d.name, min: d.thresholdMin || 0, max: d.thresholdMax || 100 }; this.showThresholdDialog = true },
    setThreshold: function() { var self = this; this.$axios.post('/api/devices/command', { deviceId: this.thresholdForm.deviceId, commandType: 'SET_THRESHOLD', commandValue: this.thresholdForm.min + ',' + this.thresholdForm.max }).then(function() { self.showThresholdDialog = false; self.$store.dispatch('fetchDevices'); self.showMsg('已设置', 'success') }) },
    openHistoryDialog: function(d) { var self = this; this.historyData = []; this.showHistoryDialog = true; this.$axios.get('/api/devices/' + d.id + '/history', { params: { page: 0, size: 10 } }).then(function(r) { if (r.data.success) self.historyData = r.data.data.content }) },
    formatDate: function(d) { return d ? new Date(d).toLocaleString('zh-CN') : '-' },
    shareDevice: function(d) { var self = this; this.$axios.post('/api/share', { deviceId: d.id, expireHours: 24 }).then(function(r) { if (r.data.success) { navigator.clipboard.writeText(window.location.origin + '/share/' + r.data.data); self.showMsg('链接已复制', 'success') } }) },
